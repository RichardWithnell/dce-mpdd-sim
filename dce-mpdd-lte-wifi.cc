...
int main (int argc, char *argv[])
{
  ...
  Ptr<c> epcHelper = CreateObject<PointToPointEpcHelper> ();
  ...
  Ptr<RateErrorModel> em1 =
    ...
    if (!disWifi) {
      ...
      LinuxStackHelper::RunIp (nodes.Get (0), Seconds (0.1), cmd_oss.str ().c_str ());
      cmd_oss.str ("");
      cmd_oss << "route add 10.1." << 0 << ".0/24 dev sim" << devices1.Get (0)->GetIfIndex () << " scope link table " << 2;
      LinuxStackHelper::RunIp (nodes.Get (0), Seconds (0.1), cmd_oss.str ().c_str ());
      cmd_oss.str ("");
      cmd_oss << "route add default via " << if1.GetAddress (1, 0) << " dev sim" << devices1.Get (0)->GetIfIndex () << " table " << 2;
      LinuxStackHelper::RunIp (nodes.Get (0), Seconds (0.1), cmd_oss.str ().c_str ());
      cmd_oss.str ("");
      ...
      if (disLte){
          cmd_oss.str ("");
          cmd_oss << "route add default via " << if1.GetAddress (1, 0) << " dev sim" << devices1.Get (0)->GetIfIndex ();
          LinuxStackHelper::RunIp (nodes.Get (0), Seconds (0.1), cmd_oss.str ().c_str ());
      }
